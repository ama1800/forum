                    ...
                    if($owner->getId() == Session::getUtilisateur()->getId() or (Session::getUtilisateur()->getRole() < 2))
                    {
                        ...
                        if(isset($_FILES['avatar']) and !empty($_FILES['avatar']['name']))
                        {
                            ...
                            if($_FILES['avatar']['size']<= $tailleMax)
                            {
                                ...
                                if(in_array($extensionUpload,$extensionValid))
                                {
                                    ...
                                    if($enregistrement)
                                    {
                                        $avatar=Session::getUtilisateur()->getId().".".$enregistrement;
                                    }
                        ...
                        $role = filter_input(INPUT_POST, "role", FILTER_SANITIZE_STRING);
                            if(($email==$email1) and !empty($email))
                            {
                                $id = (isset($_GET['id'])) ? $_GET['id'] : null;
                                $manager = new UtilisateurManager();
                                $array=['id'=>$id,"psuedo"=>$psuedo,"email"=>$email,"nom"=>$nom,"prenom"=>$prenom,"datenaissance"=>$datenaissance,"avatar"=>$avatar ];
